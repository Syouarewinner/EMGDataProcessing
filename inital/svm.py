...
def create_data():
    ...
    for i in range(len(data)):
        if data[i, -1] == 0:
            data[i, -1] = -1
    # print(data)
    return data[:, :2], data[:, -1]     # [['sepal length', 'sepal width'], 'label']
...
# %%
print('svm.fit(X_train, y_train)', model.fit(X_train, y_train))

q = np.column_stack((X_test, y_test, model.predict(X_test)))
print(q[0:20])
